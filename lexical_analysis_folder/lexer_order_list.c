...
/*   Updated: 2024/03/29 22:25:31 by atucci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_empty_redirection(t_list_of_tok **head)
{
	...
	while (current != NULL)
	{
		...
		if (is_a_redirection(current))
		{
			...
			if (is_valid_prev(prev) == 0
				&& is_valid_next(next) == 0 && after_filename != 1)
			{
...
void	swap_redirection_with_command(t_list_of_tok **head)
{
	...
	while (current)
	{
		if (is_a_redirection(current))
		{
			...
			&& file_node && my_strcmp("EMPTY", current->token) != 0)
		{
...
int	should_swap(t_list_of_tok *head)
{
	...
	while (current != NULL)
	{
		if (is_a_redirection(current) && current->next != NULL
			&& current->next->type == T_FILE_NAME)
		{
			command_node = current->next->next;
			printf("command_node token [%s]\n", command_node->token);
			if (command_node != NULL && (command_node->type == T_COMMAND
					|| command_node->type == T_BUILTIN))
			{
				next_node = command_node->next;
				if (next_node == NULL || is_a_redirection(next_node)
					|| next_node->type == T_PIPES || next_node->type == T_FLAG
					|| next_node->type == T_COMMAND_ARGS)
				{
