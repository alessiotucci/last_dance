...
/*   Updated: 2024/03/29 22:12:58 by atucci           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* 5 */
t_list_of_tok	*find_first_occurrence(t_list_of_tok *head, t_type_of_tok enmy)
{
...
/* 1 */
void	move_node(t_list_of_tok **head, t_list_of_tok *tget, t_list_of_tok *mov)
{
	t_list_of_tok	*sublist_end;

	sublist_end = mov;
	while (sublist_end->next && sublist_end->next->type != T_REDIR_OUT
		&& sublist_end->next->type != T_REDIR_IN
		&& sublist_end->next->type != T_REDIR_APP
		&& sublist_end->next->type != T_PIPES)
	...
	mov->previous = tget->previous;
	sublist_end->next = tget;
	if (tget->previous != NULL)
		tget->previous->next = mov;
	tget->previous = sublist_end;
	if (*head == tget)
		*head = mov;
}
